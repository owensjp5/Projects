...
samples_info <- data.frame(
    ...
    condition = factor(classes, level = c("control","ipf"))

)

rownames(samples_info) <- colnames(data)

dds <- DESeqDataSetFromMatrix(countData = data, colData = samples_info, design = ~ condition)
...
pheatmap(heatmap_data, scale = "row", clustering_distance = "euclidean", clustering_distance_cols = "euclidean",
                        clustering_method = "complete", annotation_col = samples_info, annotation_colors = annotation_colors,
                        color = my_colors, fontsize_row = 6, fontsize_col = 4)
